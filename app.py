...
class NewNoteForm(FlaskForm):
    xmname = TextAreaField('Xmname', validators=[DataRequired()])
    xmenvironment = TextAreaField('Xmenvironment', validators=[DataRequired()])
    xmurl = TextAreaField('Xmurl', validators=[DataRequired()])
...
class Note(db.Model):
    ...
    xmname = db.Column(db.Text)
    xmenvironment = db.Column(db.Text)
    xmurl = db.Column(db.Text)
...
def new_note():
    ...
    if form.validate_on_submit():
        xmname = form.xmname.data
        xmenvironment = form.xmenvironment.data
        xmurl = form.xmurl.data
        note = Note(xmname=xmname,xmenvironment=xmenvironment,xmurl=xmurl)
        db.session.add(note)
        db.session.commit()
...
def edit_note(note_id):
    ...
    if form.validate_on_submit():
        note.xmname = form.xmname.data
        note.xmenvironment = form.xmenvironment.data
        note.xmurl = form.xmurl.data
        db.session.commit()
        flash('Your note is updated.')
        return redirect(url_for('index'))
    form.xmname.data = note.xmname  # preset form input's value
    form.xmenvironment.data = note.xmenvironment  # preset form input's value
    form.xmurl.data = note.xmurl  # preset form input's value
